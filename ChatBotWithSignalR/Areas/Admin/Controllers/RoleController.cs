    ...
    public class RoleController : Controller
    {
        ...
        public async Task<IActionResult> CreateOrEdit(string roleId, string roleName)
        {
            try
            {
                if (string.IsNullOrEmpty(roleId))
                {
                    ...
                    await _toast.ToastSuccess("Role Created Successfully");
                ...
                else
                {
                    ...
                    if (role == null)
                    {
                        await _toast.ToastError("Role Not Found");
                        return RedirectToAction(nameof(Index));
                    }
                    else
                    {
                        ...
                        await _toast.ToastSuccess($"Role Update Successfully");
                        return RedirectToAction(nameof(Index));
                    }
                }
            }
        ...
        public async Task<IActionResult> Delete(string roleId)
        {
            try
            {
                ...
                await _toast.ToastSuccess($"Role ({role.Name}) Delete Successfully");
                return RedirectToAction(nameof(Index));
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> OnPostRoleClaims(PermissionViewModel model)
        {
            try
            {
                ...
                if (result.Succeeded)
                {
                    await _toast.ToastSuccess($"Permissions Updated Successfully");
                    return View("RoleClaims", model);
                }
                await _toast.ToastError("Role Not Found");
                return View("RoleClaims", model);
